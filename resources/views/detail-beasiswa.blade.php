...
@section('title', 'Nama Lowongan Beasiswa')
...
<div x-data="{ showModal: false, externalUrl: '' }" class="w-full">
    ...
    <div class="flex flex-col gap-4">    
        <div class="w-full flex flex-col items-start gap-2">
            <div class="w-full text-black text-4xl font-bold leading-tight">{{ $lowonganBeasiswa['title'] }}</div>
            <div class="w-full text-[#073B60] text-xl font-medium leading-tight">{{ $lowonganBeasiswa['organizer'] }}</div>
        </div>

        <div class="w-1/2 rounded-xl border border-[#E2E2E2] overflow-hidden flex flex-col items-start p-2">
            <div class="overflow-hidden flex items-center gap-1">
                ...
                <div class="text-[#073B60] text-base font-normal">{{ $lowonganBeasiswa['start_period_date'] }} - {{ $lowonganBeasiswa['end_period_date'] }}</div>
            </div>
        </div>
        ...
        <div class="w-full flex flex-col items-center">
            <img class="w-1/2" src="{{ $lowonganBeasiswa['banner'] }}" alt="Lowongan Kerja">
        </div>
        ...
        <div class="w-full flex flex-col items-start gap-1 border border-gray-200 rounded-lg">
            <div class="w-full p-2 flex items-start flex-col gap-1">
                ...
                <div class="text-[#073B60] text-base font-normal leading-snug">{{ $lowonganBeasiswa['description'] }}</div>
            </div>
            <div class="w-full p-2 flex items-start flex-col gap-1">
                <div class="text-[#073B60] text-base font-bold leading-tight">Manfaat</div>
                <div class="text-[#073B60] text-base font-normal leading-snug">{{ $lowonganBeasiswa['benefits'] }}</div>
            </div>
            <div class="w-full p-2 flex items-start flex-col gap-1">
                <div class="text-[#073B60] text-base font-bold leading-tight">Persyaratan</div>
                <div class="text-[#073B60] text-base font-normal leading-snug">{{ $lowonganBeasiswa['requirements'] }}</div>
            </div>
            <div class="w-full p-2 flex items-start flex-col gap-1">
                <div class="text-[#073B60] text-base font-bold leading-tight">Dokumen yang Dibutuhkan</div>
                <div class="text-[#073B60] text-base font-normal leading-snug">{{ $lowonganBeasiswa['file_collections'] }}</div>
            </div>
        </div>
        <button class="w-full bg-[#1080CF] text-white text-base font-normal leading-snug rounded-lg flex flex-col items-center p-3 hover:bg-[#0d6faf]"
            @click="externalUrl = '{{ $lowonganBeasiswa['registration'] }}'; showModal = true;">Daftar Sekarang
        </button>
    </div>
</div>
@endsection
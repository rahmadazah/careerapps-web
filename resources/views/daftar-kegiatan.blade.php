...
@section('content')
    <h1 class="w-full text-black text-3xl font-bold leading-tight">Kegiatan</h1>
    <div class="w-full flex flex-col md:flex-row justify-between items-center gap-3">
        <div class="flex gap-2 text-sm items-center flex-wrap">
            ...
            <a href="{{ route('kegiatan') }}"
                class="p-2 rounded-lg {{ empty($filterType) ? 'bg-[#1080CF] text-white' : 'bg-white text-[#777] border border-[#777]' }}">
                Terbaru
            </a>
            <a href="{{ route('kegiatan', ['type' => 'online', 'search' => request('search')]) }}"
                class="p-2 rounded-lg {{ $filterType === 'online' ? 'bg-[#1080CF] text-white' : 'bg-white text-[#777] border border-[#777]' }}">
                Online
            </a>
            <a href="{{ route('kegiatan', ['type' => 'offline', 'search' => request('search')]) }}"
                class="p-2 rounded-lg {{ $filterType === 'offline' ? 'bg-[#1080CF] text-white' : 'bg-white text-[#777] border border-[#777]' }}">
                Offline
            </a>
        </div>
    ...
    @else
        <ul class="w-full flex flex-col gap-4 mt-4">
            @foreach ($daftarKegiatan as $kegiatan)
                <li>
                    <a href="{{ route('kegiatan.detail', $kegiatan['slug']) }}" class="w-full p-4 bg-white rounded-xl border border-[#E2E2E2] flex flex-row items-center gap-5">
                        <div class="w-20 h-20 overflow-hidden flex items-center justify-center">
                            <img class="max-w-full max-h-full object-contain" src="{{ $kegiatan['banner'] }}" />
                        </div>
                        <div class="flex flex-col items-start rounded gap-1">
                            <div class="w-full text-xl font-bold">{{ $kegiatan['title'] }}</div>
                            <div class="w-full text-base font-medium">{{ $kegiatan['organizer'] }}</div>
                            <div class="w-full flex items-center gap-1">
                                <div class="h-4 flex items-center gap-1 overflow-hidden">
                                    ...
                                    <div class="text-[#073B60] text-xs font-normal">{{ $kegiatan['type'] }}</div>
                                </div>
                            </div>
                            <div class="w-full text-[#777] text-xs font-normal">{{ $kegiatan['createdAt'] }}</div>
                        </div>
                    </a>
                </li>
            @endforeach
        ...
        <div class="w-full flex items-center justify-center gap-2 select-none">
            @if ($currentPage > 1)
                <a href="?page={{ $currentPage - 1 }}&type={{ $filterType }}&search={{ $search }}" 
            ...
            @for ($i = 1; $i <= $totalPages; $i++)
                <a href="?page={{ $i }}&type={{ $filterType }}&search={{ $search }}"
